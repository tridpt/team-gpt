...
chương trình gửi một _yêu cầu_ (request) và nhận về một _phản hồi_ (response).
...
**HTTP & endpoint**
HTTP là "ngôn ngữ" client và server dùng để nói chuyện qua mạng. Một _endpoint_
...
một chiều của mật khẩu (gọi là _hash_). Từ hash không thể suy ngược ra mật khẩu.
...
_streaming_. TeamGPT dùng kỹ thuật **SSE (Server-Sent Events)**: server giữ kết
...
- **Express**: một thư viện giúp viết server web bằng Node dễ hơn (định nghĩa các
  endpoint, xử lý request...). Đây là thư viện phụ thuộc _duy nhất_ của TeamGPT.
...
**Nguyên tắc thiết kế xuyên suốt:** _tách bạch trách nhiệm_ (mỗi file làm một
việc rõ ràng) và _tối giản phụ thuộc_ (chỉ dùng Express + các thứ có sẵn trong
...
- **scrypt** là thuật toán băm cố tình _chậm và tốn bộ nhớ_ → kẻ tấn công có lấy
...
- Không cho hạ quyền/vô hiệu hóa **admin hoạt động cuối cùng** (tránh lỡ tay
  khóa mình ra khỏi hệ thống — _lockout_).
...
- **Cắt ngữ cảnh** (`contextMessages`): chỉ gửi **20 tin gần nhất** lên AI. Vì
  ...
  Tin gần đây mang nhiều thông tin liên quan nhất. Đây là chiến lược _sliding
  window_.
...
**Vì sao trước đây TeamGPT phải tự ước lượng chi phí, và giờ thì sao?**
Luồng chat là **streaming**. Trước đây SSE của gateway không kèm token, nên
TeamGPT ước lượng (~4 ký tự/token). Giờ TeamGPT gửi `stream_options.include_usage`
và gateway phát một **chunk usage cuối stream** với token thật — TeamGPT dùng số
đó, chỉ **fallback về ước lượng** khi gateway không cung cấp. Frame `done` có cờ
`estimated` cho biết con số là thật hay ước lượng.
...
**Vì sao phải tự gom buffer rồi tách theo `\n\n`?** Vì dữ liệu về theo _khối byte
tùy ý_, không trùng ranh giới frame. Một frame có thể bị cắt đôi giữa 2 khối,
...
- Lỗi _trước khi_ gửi header → trả JSON lỗi kèm mã status bình thường.
- Lỗi _sau khi_ đã bắt đầu stream → chỉ còn cách gửi frame `{"type":"error"}` rồi
  đóng, vì mã status đã trót gửi đi rồi (không sửa lại được).

**Giữ kết nối & dừng giữa chừng:**

- Server gửi **heartbeat** (dòng comment `: ping`) mỗi 15 giây để proxy không
...
**Hiển thị Markdown:** câu trả lời của trợ lý được render Markdown (tiêu đề, danh
sách, **đậm**, `code`, khối code, blockquote, link) bằng một bộ render nhỏ tự
viết trong `app.js`. Bộ render **escape HTML trước** rồi mới áp dụng Markdown, nên
nội dung model trả về **không thể chèn HTML độc** (chống XSS).
...
cảnh 20 tin gần nhất, đổi tên, đổi model (`setModel`), xóa. Mọi hàm đều kiểm tra
quyền sở hữu.
...
| Method   | Path                                | Thân gửi                                     | Trả về                                                                                                                    |
| -------- | ----------------------------------- | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `GET`    | `/api/conversations`                | — (tùy chọn `?q=`, `?limit=`, `?offset=`)    | `{ conversations, total, hasMore, usage, limits }`. `?q=` lọc; `?limit/offset` phân trang.                                |
...
| `POST`   | `/api/admin/users`           | `{ email, name, password, role, budget, defaultModel, groupId }`           | User vừa tạo (`201`). `400` nếu dữ liệu sai / `groupId` lạ. |
| `GET`    | `/api/admin/users/:id`       | —                                                                          | `{ user, limits, usage }`                                   |
| `PATCH`  | `/api/admin/users/:id`       | `{ name?, role?, password?, disabled?, budget?, defaultModel?, groupId? }` | User đã cập nhật.                                           |
| `DELETE` | `/api/admin/users/:id`       | —                                                                          | `{ ok: true }` + xóa hội thoại/usage/phiên của họ.          |
| `GET`    | `/api/admin/groups`          | —                                                                          | `{ groups }` (kèm usage hôm nay + số thành viên mỗi nhóm).  |
| `POST`   | `/api/admin/groups`          | `{ name, budget }`                                                         | Nhóm vừa tạo (`201`).                                       |
| `PATCH`  | `/api/admin/groups/:id`      | `{ name?, budget? }`                                                       | Nhóm đã cập nhật.                                           |
| `DELETE` | `/api/admin/groups/:id`      | —                                                                          | `{ ok: true }` + gỡ nhóm khỏi mọi thành viên.               |
...
**Nhóm / phòng ban:** mỗi user thuộc **0 hoặc 1 nhóm**. Nhóm có hạn mức ngày
(số request + chi phí) **dùng chung cho mọi thành viên** — được kiểm tra **cộng
thêm** vào hạn mức cá nhân. Khi bất kỳ ai trong nhóm gửi tin, tổng usage hôm nay
của cả nhóm được cộng lại; vượt hạn mức nhóm → `429` cho mọi thành viên (dù hạn
mức cá nhân của họ chưa chạm).

**Đính kèm file văn bản:** người dùng đính kèm tối đa 5 file văn bản (mỗi file
≤ 128 KB), đọc **phía trình duyệt** và chèn vào prompt dưới dạng khối mã có nhãn
tên file. Nhờ đó **mọi model** (kể cả không đa phương thức) đều dùng được nội
dung file. Server không lưu file — chỉ nhận nội dung như một phần tin nhắn.
...
- `streaming.test.js` — **test hồi quy streaming**: dựng một _fake gateway_ trả
...
- Chi phí là **ước lượng** khi gateway không gửi usage (thường thì có usage thật).
- Bộ render Markdown là bản tối giản tự viết (không hỗ trợ mọi cú pháp GFM).

**Hướng phát triển tiếp:**

- Chuyển sang **DB thật** (SQLite/Postgres) để chịu tải và chạy nhiều instance.
- **Đính kèm ảnh** cho model đa phương thức (hiện mới hỗ trợ file văn bản); tô
  màu cú pháp nâng cao.
- Hạn mức nhiều tầng hơn (theo tuần/tháng), báo cáo usage theo nhóm.
