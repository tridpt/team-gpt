...
| `POST` | `/api/admin/users` | `{ email, name, password, role, budget, defaultModel }` | User vừa tạo (`201`). `400` nếu dữ liệu sai. |
| `GET` | `/api/admin/users/:id` | — | `{ user, limits, usage }` |
| `PATCH` | `/api/admin/users/:id` | `{ name?, role?, password?, disabled?, budget?, defaultModel? }` | User đã cập nhật. |
...
- **Đính kèm file/hình** (cần model đa phương thức); tô màu cú pháp nâng cao.
